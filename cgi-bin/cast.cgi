...
eval `/usr/lib/cgi-bin/proccgi.sh $*`
echo ""
#echo $FORM_host
echo $FORM_vid
...
{
    if ((GUI)); then
        zenity --error --ellipsize --text "$FORM_vid"
    else
        printf '\n%s\n' "$FORM_vid" >&2
    fi
...
{
    if ((GUI)); then
        zenity --question --ellipsize --text "$FORM_vid"
    else
        printf '\n%s [y/N] ' "$FORM_vid" >&2
        read
        [[ $REPLY =~ y|Y ]]
...
download_and_serve()
{
    ...
    TMP_FILE="$(youtube-dl --get-filename "$FORM_vid")"
    ...
    if ((GUI)); then
        ...
        zenity --progress --auto-close --text "Downloading video..." < <(youtube-dl -o "$file" --newline "$FORM_vid" | sed -Eun 's/.* ([0-9][0-9]?)\.[0-9]%.*/\1/p') || exit
    else
        youtube-dl -o "$file" "$FORM_vid"
    fi
...
serve()
{
    ...
    if [[ $FORM_vid ]]; then
        ln -s "$(realpath "$FORM_vid")" "$TMP_DIR/media" || error "Failed to write to shared directory"
    fi
